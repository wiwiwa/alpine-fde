...
1. **The root filesystem is fully encrypted** (LUKS2) and **minimal** (~200 MB installed, §3.2).
...
- G7 (Minimal Footprint) — The installed system is **minimal** (§3.2): only what boot, unlock, audit, and admin need (~200 MB budget).
...
- **Size budget:** Initial planning target ≤ 250 MB installed.
...
| ADR-12 | Minimal rootfs: `apk add --root`, explicit additions set, size budget ≤ 250 MB asserted in CI | User requirement; §3.2 is the normative recipe |
