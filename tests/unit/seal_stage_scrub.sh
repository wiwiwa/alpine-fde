#!/usr/bin/env bash
# tests/unit/seal_stage_scrub.sh — §11 I1 seal/enroll staging hygiene:
#   * the staging root is TMPFS by construction: with DEBIAN_FDE_TMPDIR unset
...
#     artifacts (debian-fde-seal-pass.* / debian-fde-seal* / debian-fde-enroll.*)
...
# TMP is the TEST TREE scanned for leftovers; DEBIAN_FDE_TMPDIR binds every
# staging root to it so the scan is complete (nothing escapes to /tmp or /dev/shm)
TMP=$(mktemp -d /tmp/debian-fde-stage-scrub.XXXXXX)
cleanup() {
    swtpm_cleanup_all
    rm -rf "$TMP"
}
...
openssl req -new -x509 -key "$KEYDIR/release.pem" -out "$KEYDIR/release.crt" \
    -subj /CN=debian-fde-stage-scrub 2>/dev/null
...
export DEBIAN_FDE_LUKS_KEYFILE=$TMP/k0
...
leftovers() {
    find "$TMP" \( -name 'debian-fde-seal-pass.*' -o -name 'debian-fde-seal*' \
        -o -name 'debian-fde-enroll.*' \) -print 2>/dev/null
}
...
assert_eq "staging root with DEBIAN_FDE_TMPDIR unset -> /dev/shm (tmpfs, never /tmp)" \
    "/dev/shm" "$(unset DEBIAN_FDE_TMPDIR; seal_stage_dir 2>/dev/null)"
assert_eq "staging root honors DEBIAN_FDE_TMPDIR" "$TMP/tmp" "$(seal_stage_dir)"
...
mk_luks "$LUKS2"
DEBIAN_FDE_CRYPTSETUP=$TMP/cs-no-addkey
ENRL2_RC=0
...
mk_luks "$LUKS4"
DEBIAN_FDE_CRYPTSETUP=$TMP/cs-no-import
UPG2_RC=0
