...
# G-T13/G-E9 (boot reaches the UKI stub): DA lockout gates authValue ops only
# — the auth-less phase-word extend still happened, so the PRE-UNLOCK PCR 11
# reading must equal the fallback UKI's signed prediction.
_CONSOLE_SAVE="$CONSOLE"
CONSOLE="$B/console.log"
assert_pcr11_prediction "S-09"
