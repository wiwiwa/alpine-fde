...
_feed_3_strike() {
    ...
    for n in 1 2 3; do
        if uki_wait_hook_prompt "$n" 300 "$d"; then
            ...
            feed_line "$d/serial.sock" "debian-fde-wrong-passphrase-$n"
        else
...
grep -q "debian-fde: UNSEALED" "$CONSOLE" || { echo "s03: baseline boot did not reach UNSEALED"; exit 1; }
...
PCR7_CONSOLE=$(grep -oE 'debian-fde-pcr sha256:7=[0-9a-f]{64}' "$RUN/console-v1-enroll.log" | head -1 | cut -d= -f2)
