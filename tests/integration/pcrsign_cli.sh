#!/usr/bin/env bash
# tests/integration/pcrsign_cli.sh — G-B4: `alpine-fde pcrsign` (§6.1.1 signer contract)
...
# Deterministic fixture initrd, generated IN-SUITE. The former shared input,
# fixtures/uki/initrd.img, was a GIT-IGNORED untracked artifact: a fresh
# checkout or worktree silently lacked it and the real ukify build below died
# ("could not build the fixture UKI"). ukify measures the bytes and never
# parses them, so fixed in-suite content is equivalent — and self-contained.
