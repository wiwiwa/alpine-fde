#!/usr/bin/env bash
# tests/integration/interop_oracle_mechb.sh — ADR-19/§12 interop oracle: upstream
...
if [[ "${ALPINE_FDE_INTEROP_ORACLE:-}" != "1" ]]; then
    ...
    # TAP skip, not silence: the runners count a file that emits no assertion
    # as a VACUOUS PASS (a failure). A gated-off suite must say so on the TAP
    # channel so the skip stays visible without failing every runner run.
    echo "ok 1 - interop oracle gated off (ALPINE_FDE_INTEROP_ORACLE != 1) # SKIP ADR-19 scope guard"
    exit 0
fi
